...
Success created the next problem: three roughly-20 GB tenants ended up sharing one 24 GB card, none of them aware of the others, so whichever loaded second died. They now sit behind **one door** that owns GPU admission — a proxy rather than a lease service, because a lease is arbitration by cooperation (anything that can reach a backend directly walks past it) while a proxy makes arbitration a property of the topology. Backends bind localhost and are unreachable except through the arbiter; admission goes by memory budget rather than a binary mutex, so an 84 MB embedder never queues behind an image model; a heavy request waits a short bounded time and then gets a `503` naming the holder with a `Retry-After`, because the caller deciding for itself beats parking on an open socket behind a 100-minute render. The door also supplies the lifecycle that one tenant lacks entirely — idle reap, reclaim-on-demand, lease-expiry — and degrades honestly, reporting per-capability `available`/`managed` with the specific dependency when it can route to a tenant but not manage it. Details in [patterns/gpu-tenancy.md](patterns/gpu-tenancy.md).
