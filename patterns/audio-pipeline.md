...
This thesis has the shape of a claim under test, not a proven conclusion. The hand-rewrite experiments that informed it produced noticeably better output than the strip-and-go baseline. The still-unvalidated link is whether an automated LLM rewrite pass can match a careful hand-rewrite. That's the next thing to falsify: add a `--rewrite` flag, run the same source through both paths, compare.
